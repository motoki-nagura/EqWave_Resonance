"""
   ...
   ω1 = 2π/(180 days), ω2 = 2π/(180 days), ω3 = 2π/(90 days) are fixed.
"""
...
#cluster_eps, cluster_threshold_ratio, cluster_crop_and_pad = 2.5, 0.2, False
#                                #  Parameters for clustering
...
ω1, ω2 = ω_180dy, ω_180dy
ω3 = ω1 + ω2              #  ω3 is 2π/(90 days)
...
match TARGET_WAVE3:
    case "Rossby":
        k3_target_in,   m3_target_in   = [-2.0e-5, 0.0e-5], [-0.8e-2, 0.8e-2] #  include
        k3_target_out1, m3_target_out1 = [-0.5e-5, 0.0e-5], [ 0.5e-2, 0.8e-2] #  exclude
        k3_target_out2, m3_target_out2 = [-0.7e-5, 0.0e-5], [ 0.6e-2, 0.8e-2] #  exclude
    case "Rossby_m<0":
        k3_target, m3_target = [-0.80e-5,-0.50e-5], [-0.65e-2,-0.05e-2]
...
OUTPUT_TXT = OUTPUT_DIR+"out_90dy_"+TARGET_WAVE3+"_"+FILTER+".txt"
OUTPUT_NC  = OUTPUT_DIR+"out_90dy_"+TARGET_WAVE3+"_"+FILTER+".nc"
OUTPUT_PDF = OUTPUT_DIR+"fig_90dy_"+TARGET_WAVE3+"_"+FILTER+".pdf"

if TARGET_WAVE3 == "Rossby" and FILTER == "3ptBox":
    OUTPUT_LIST_NC = OUTPUT_DIR+"out_list_90dy_"+TARGET_WAVE3+"_"+FILTER+".nc"
...
def main():
    ...
    BSR_ω1ω2_cond, im3_cond, ik3_cond = \
                         Eliminate_BSR_Range("outside",BSR_ω1ω2_cond, m3_table,k3_table, m3_target_in,  k3_target_in)
    BSR_ω1ω2_cond, _,_ = Eliminate_BSR_Range("inside", BSR_ω1ω2_cond, m3_table,k3_table, m3_target_out1,k3_target_out1)
    BSR_ω1ω2_cond, _,_ = Eliminate_BSR_Range("inside", BSR_ω1ω2_cond, m3_table,k3_table, m3_target_out2,k3_target_out2)
    ...
    BSR_ω1ω2_m1k1, BSR_ω1ω2_m2k2, BSR_ω1ω2_m3k3 = \
        Substitute_Listed_Values_in_km_space(BSR_ω1ω2,m_BSR,k_BSR, list_TopNValues,
                                             m3_table,k3_table, m3_target_in,k3_target_in)
    ...
    if TARGET_WAVE3 == "Rossby" and FILTER == "3ptBox":
        ...
        Save_BSR_BC_to_NetCDF(
                ...
                groups_cluster,scale_BSR_ω1ω2,m3_target_in,k3_target_in)
    ...
    Draw_Figure_to_PDF(OUTPUT_PDF,
                       ...
                       m_range,k_range,ω1,ω2,ω3,m3_target_in,k3_target_in,
                       ttl_periods=("180","90","90"),
                       )
