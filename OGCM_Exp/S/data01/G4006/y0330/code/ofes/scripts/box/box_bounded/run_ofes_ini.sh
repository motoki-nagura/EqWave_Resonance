...
# &contrl init=.true., runlen=4.0, rununits='months',           #  4 months
# &contrl init=.true., runlen=3.0, rununits='days',             #  3 days
# &contrl init=.true., runlen=0.000694444444, rununits='days',  #  60 seconds
# &contrl init=.true., runlen=12.0, rununits='months',          #  1 year
cat > namelist.contrl << ENDNAMELIST
 &contrl init=.true., runlen=12.0, rununits='months',
...
# time steps
#======================================================================
# &tsteps dtts=20., dtuv=20., dtsf=0.4,   #  for test
# &tsteps dtts=100., dtuv=100., dtsf=2.,  #  default for 0.1 deg resolution
cat > namelist.tsteps << ENDNAMELIST
 &tsteps dtts=100., dtuv=100., dtsf=2.0,
 /
ENDNAMELIST
...
# &mixing am=0.0e0, ah=0.0e0, ambi=0.3e18, ahbi=1.e17,
...
#  ----------> From here (added, nagura, 2024/11/7)
...
cat > namelist.ppmix << ENDNAMELIST
 ...
 /
ENDNAMELIST
#  <---------- Until here (added, nagura, 2024/11/7)

#  ----------> From here (commented out, nagura, 2024/11/7)
#cat > namelist.ppmix << ENDNAMELIST
# &ppmix wndmix=10.0,  
# /
#ENDNAMELIST
#  <---------- Until here (commented out, nagura, 2024/11/7)
